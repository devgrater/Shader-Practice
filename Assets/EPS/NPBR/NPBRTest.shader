    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            fixed4 frag (v2f i) : SV_Target
            {
                ...
                lighting = smoothstep(0.1f, 0.45f, lighting);
                
                //return dfg_d(nDotH, roughness);//highlight?
                ...
                fixed3 rimlight = 1 - saturate(nDotV);
                rimlight = pow(rimlight, 4) * f0;

                float4 composite = float4(direct * lightAmount * f0 + indirect + rimlight, 1.0);//float4(direct * lightAmount + indirect, 1.0);// + saturate(pow(1.0 - nDotV, 6) * (1 - vDotH)), 1.0);
