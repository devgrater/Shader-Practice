Shader "Grater/GraterNPBR"
{
    Properties
    {
        _SideTex ("Texture", 2D) = "white" {}
        _UVScaling ("Triplanar Scaling", float) = 1.0
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            fixed4 frag (v2f i) : SV_Target
            {
                ...
                fixed3 absNormal = abs(normalize(i.normal));
                absNormal *= absNormal;

                //texture:
                //Y direction
                fixed4 yColor = tex2D(_MainTex, i.worldPos.xz * _UVScaling);
                fixed4 xColor = tex2D(_SideTex, i.worldPos.yz * _UVScaling);
                fixed4 zColor = tex2D(_SideTex, i.worldPos.xy * _UVScaling);

                fixed4 col = xColor * absNormal.x + yColor * absNormal.y + zColor * absNormal.z * _Color;
